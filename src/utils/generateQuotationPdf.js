 ...
 * @param {string} [options.nextInvoiceDate]
...
export function generateQuotationPdf(options = {}) {
  ...
  if (nextInvoiceDate) {
    infoY += 6
    doc.text(`Next Invoice Date: ${nextInvoiceDate}`, 14, infoY)
  }
  infoY += 10
  ...
  if (products.length > 0) {
    ...
    const tableBody = products.map((p) => [
      ...
      formatCurrencyINR(p.total_amount ?? p.totalAmount ?? 0),
    ])

    const grandTotal = products.reduce((sum, p) => {
      return sum + Number(p.total_amount ?? p.totalAmount ?? 0)
    }, 0)
  ...
  doc.text(
    'This quotation is prepared for your review. Please accept or reject from your dashboard.',
    14,
    pageHeight - 16,
  )

  doc.save(`Quotation-${subscriptionNumber}.pdf`)
}
