...
import { getTaxById, updateTax } from '../../../../services/taxApi'
...
function TaxesEditPage() {
  ...
  useEffect(() => {
    ...
    const loadTax = async () => {
      ...
      try {
        ...
        setTaxName(String(tax?.tax_name ?? ''))
        setTaxComputationUnit(String(tax?.tax_computation_unit ?? 'Fixed Price'))
        setTaxComputationValue(String(tax?.tax_computation_value ?? ''))
      } catch (error) {
        if (!isMounted) {
          return
        }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setToastMessage(response?.message ?? 'Tax updated successfully.')

      const tax = response?.tax
      if (tax) {
        setTaxName(String(tax.tax_name ?? normalizedTaxName))
        setTaxComputationUnit(String(tax.tax_computation_unit ?? taxComputationUnit))
        setTaxComputationValue(String(tax.tax_computation_value ?? normalizedValue))
      }
    } catch (error) {
      setToastVariant('error')
      setToastMessage(error.message)
  ...
  return (
    <div className="rounded-2xl border border-[color:rgba(0,0,128,0.14)] bg-[var(--white)] p-8 sm:p-10">
      ...
      <p className="mt-2 text-sm text-[color:rgba(0,0,128,0.76)]">
        Update tax details, computation unit, and value.
      </p>
