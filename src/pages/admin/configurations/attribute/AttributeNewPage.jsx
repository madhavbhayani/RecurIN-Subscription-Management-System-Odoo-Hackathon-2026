...
import { createAttribute } from '../../../../services/attributeApi'
...
function AttributeNewPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setToastMessage(response?.message ?? 'Attribute created successfully.')

      setAttributeName('')
      setValueRows([createValueRow()])
    } catch (error) {
      setToastVariant('error')
  ...
  return (
    <div className="rounded-2xl border border-[color:rgba(0,0,128,0.14)] bg-[var(--white)] p-8 sm:p-10">
      ...
      <p className="mt-2 text-sm text-[color:rgba(0,0,128,0.76)]">
        Create an attribute and define one or more values with default extra pricing.
      </p>
            ...
            <table className="min-w-full border-separate border-spacing-0 text-sm">
              ...
              <tbody>
                {valueRows.map((row, index) => (
                  <tr key={`attribute-value-row-${index}`}>
...
export default AttributeNewPage