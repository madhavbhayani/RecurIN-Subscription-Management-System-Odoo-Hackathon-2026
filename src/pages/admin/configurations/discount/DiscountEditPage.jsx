...
function DiscountEditPage() {
  ...
  const [appliedUserCount, setAppliedUserCount] = useState(0)
  ...
  useEffect(() => {
    ...
    const loadDiscountAndProducts = async () => {
      setIsLoading(true)
      try {
        const [productsResponse, discountResponse] = await Promise.all([
          listProducts(''),
        ...
        setProductOptions(mapProductsToOptions(productsResponse?.products))
        setDiscountName(String(discount?.discount_name ?? ''))
        setDiscountUnit(String(discount?.discount_unit ?? 'Percentage'))
        setDiscountValue(String(discount?.discount_value ?? ''))
        setMinimumPurchase(String(discount?.minimum_purchase ?? '0'))
        setMaximumPurchase(String(discount?.maximum_purchase ?? '0'))
        setStartDate(String(discount?.start_date ?? ''))
        setEndDate(String(discount?.end_date ?? ''))
        setIsLimit(Boolean(discount?.is_limit))
        setLimitUsers(
          discount?.limit_users !== null && discount?.limit_users !== undefined
            ? String(discount.limit_users)
            : ''
        )
        setAppliedUserCount(Number(discount?.applied_user_count ?? 0))
        setStatus(String(discount?.status ?? 'inactive').toLowerCase() === 'active' ? 'active' : 'inactive')

        const incomingProductIDs = Array.isArray(discount?.products)
          ? discount.products
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await updateDiscount(discountId, {
      ...
      setToastMessage(response?.message ?? 'Discount updated successfully.')

      const updatedDiscount = response?.discount
      if (updatedDiscount) {
        setDiscountName(String(updatedDiscount?.discount_name ?? normalizedDiscountName))
        setDiscountUnit(String(updatedDiscount?.discount_unit ?? discountUnit))
        setDiscountValue(String(updatedDiscount?.discount_value ?? parsedDiscountValue))
        setMinimumPurchase(String(updatedDiscount?.minimum_purchase ?? parsedMinimumPurchase))
        setMaximumPurchase(String(updatedDiscount?.maximum_purchase ?? parsedMaximumPurchase))
        setStartDate(String(updatedDiscount?.start_date ?? startDate))
        setEndDate(String(updatedDiscount?.end_date ?? endDate))
        setIsLimit(Boolean(updatedDiscount?.is_limit))
        setLimitUsers(
          updatedDiscount?.limit_users !== null && updatedDiscount?.limit_users !== undefined
            ? String(updatedDiscount.limit_users)
            : ''
        )
        setAppliedUserCount(Number(updatedDiscount?.applied_user_count ?? appliedUserCount))
        setStatus(String(updatedDiscount?.status ?? 'inactive').toLowerCase() === 'active' ? 'active' : 'inactive')

        const updatedProductIDs = Array.isArray(updatedDiscount?.products)
          ? updatedDiscount.products
            .map((product) => String(product?.product_id ?? '').trim())
            .filter((productID) => productID !== '')
          : selectedProductIDs
  ...
  return (
    <div className="mx-auto max-w-5xl rounded-2xl border border-[color:rgba(0,0,128,0.14)] bg-[var(--white)] p-5 sm:p-8 lg:p-10">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="[font-family:var(--font-display)] text-3xl font-bold text-[var(--navy)]">Edit Discount</h1>
        <Link
      ...
      <p className="mt-2 text-sm text-[color:rgba(0,0,128,0.76)]">
        Update discount rules, product applicability, usage limits, and status.
      </p>
        ...
        <form className="mt-6 space-y-6" onSubmit={handleSubmit} noValidate>
          ...
          {isLimit ? (
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-[color:rgba(0,0,128,0.62)]">
                Applied users count: {appliedUserCount}
              </p>
            </div>
          ) : null}
