...
import { getPaymentTermById, updatePaymentTerm } from '../../../../services/paymentTermApi'
...
function PaymentTermEditPage() {
  ...
  useEffect(() => {
    ...
    const loadPaymentTerm = async () => {
      setIsLoading(true)
      try {
        const response = await getPaymentTermById(paymentTermId)
        if (!isMounted) {
          return
        }

        const paymentTerm = response?.payment_term
        setPaymentTermName(String(paymentTerm?.payment_term_name ?? ''))
        setDueUnit(String(paymentTerm?.due_unit ?? 'Percentage'))
        setDueValue(String(paymentTerm?.due_value ?? ''))
        setIntervalDays(String(paymentTerm?.interval_days ?? ''))
      } catch (error) {
        if (!isMounted) {
          return
        }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await updatePaymentTerm(paymentTermId, {
      ...
      setToastMessage(response?.message ?? 'Payment term updated successfully.')

      const updatedPaymentTerm = response?.payment_term
      if (updatedPaymentTerm) {
        setPaymentTermName(String(updatedPaymentTerm?.payment_term_name ?? normalizedPaymentTermName))
        setDueUnit(String(updatedPaymentTerm?.due_unit ?? dueUnit))
        setDueValue(String(updatedPaymentTerm?.due_value ?? parsedDueValue))
        setIntervalDays(String(updatedPaymentTerm?.interval_days ?? parsedIntervalDays))
      }
    } catch (error) {
      setToastVariant('error')
      setToastMessage(error.message)
  ...
  return (
    <div className="rounded-2xl border border-[color:rgba(0,0,128,0.14)] bg-[var(--white)] p-8 sm:p-10">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="[font-family:var(--font-display)] text-3xl font-bold text-[var(--navy)]">Edit Payment Term</h1>
        <Link
      ...
      <p className="mt-2 text-sm text-[color:rgba(0,0,128,0.76)]">
        Update due unit, amount, and interval after quotation acceptance.
      </p>
      ...
      {isLoading ? (
        ...
        <form className="mt-6 space-y-6" onSubmit={handleSubmit} noValidate>
          ...
          <div className="flex flex-wrap items-center justify-end gap-3">
            ...
            >
              {isSubmitting ? 'Updating...' : 'Update Payment Term'}
            </button>
          </div>
        </form>
      )}
