...
import { createQuotationTemplate } from '../../../../services/quotationTemplateApi'
...
function QuotationTemplateNewPage() {
  ...
  const [isLoadingRecurringPlans, setIsLoadingRecurringPlans] = useState(true)
  ...
  useEffect(() => {
    ...
    const loadRecurringPlansAndProducts = async () => {
      setIsLoadingRecurringPlans(true)
      setIsLoadingProducts(true)
      try {
        const [recurringResponse, productResponse] = await Promise.all([
        ...
        setRecurringPlans(Array.isArray(recurringResponse?.recurring_plans) ? recurringResponse.recurring_plans : [])
        const productItems = Array.isArray(productResponse?.products) ? productResponse.products : []
        setProducts(productItems)
        setProductOptions(mapProductsToOptions(productItems))
      } catch (error) {
        if (!isMounted) {
          return
        }
        ...
        if (isMounted) {
          setIsLoadingRecurringPlans(false)
          setIsLoadingProducts(false)
        }
      }
    }

    loadRecurringPlansAndProducts()
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await createQuotationTemplate({
      ...
      setToastMessage(response?.message ?? 'Quotation created successfully.')

      setLastForever(false)
  ...
  return (
    <div className="rounded-2xl border border-[color:rgba(0,0,128,0.14)] bg-[var(--white)] p-8 sm:p-10">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="[font-family:var(--font-display)] text-3xl font-bold text-[var(--navy)]">New Quotation Template</h1>
        <Link
      ...
      <p className="mt-2 text-sm text-[color:rgba(0,0,128,0.76)]">
        Configure validity behavior, link recurring plan, and select products for this quotation template.
      </p>
...
export default QuotationTemplateNewPage
